...
Native Tool Support Plan With AI SDK Tool Policies

## Goal

Support tools through the Vercel AI SDK runtime while keeping Sprint 001 intentionally limited to provider-native and app-defined AI SDK tools, all controlled through model cards, agent settings, and handler hooks, with clear clean-architecture boundaries and test expectations.
...
3. Define `ModelToolPolicy`.
   ...
   - Keep tool policy declarative so it can be evaluated in the application layer and materialized in infrastructure.
4. Define normalized tool events in shared/application contracts.
   - `tool-call-started`
   - `tool-call-output`
   - `tool-call-finished`
   - `tool-call-failed`
...
5. Update handler contracts for AI SDK tool execution.
   - Accept a tool-policy input derived from shared metadata and agent state.
...
6. Implement model-card-controlled tool shaping.
   - Allow model cards to:
...
7. Plan transcript persistence in detail.
...
- AI SDK infrastructure tests should verify:
  ...
  - model-card-driven tool shaping
...
- UI tests should verify:
  ...
  - unsupported tool states are hidden or disabled from `ModelCard.uiPresentation`
  - transcript rendering remains stable around interleaved text and tool events

## Acceptance Criteria

- Tool support is controlled by the combination of `ModelCard.capabilities.tools`, agent `toolsEnabled`, and model-card hook output.
...
- Model cards can add, remove, or reshape tools without introducing provider-specific branching in the UI.
- Tool activity is visible in the transcript after streaming and after reload.

## Dependencies

- `01-model-card-handler.md` provides `ModelCard`, `ModelToolPolicy`, and handler-hook contracts.
- `02-backend-apis-services.md` provides AI SDK-backed run persistence and event storage.
...
- Tool semantics vary across providers and models, so the model-card policy layer must stay authoritative.
