<script lang="ts">
  ...
  const slipPct = $derived(Math.min(100, Math.abs(wheel.slipRatio) * 100));
...
<div class="panel wheel-card" data-testid="hud-wheel-{wheel.index}" class:airborne={wheel.airborne}>
  <div class="head">
    ...
    <span class="surf">{wheel.surface ?? 'AIR'}</span>
  </div>
  <div class="row">
    <span class="label2">Fz</span>
    <div class="meter fz"><span style="width: {fzPct}%"></span></div>
    <span class="v">{Math.round(wheel.fz)}</span>
  </div>
  <div class="row">
    <span class="label2">Slip</span>
    <div class="meter slip"><span style="width: {slipPct}%"></span></div>
    <span class="v">{(wheel.slipRatio * 100).toFixed(0)}%</span>
  </div>
  <div class="row">
    <span class="label2">Tire °C</span>
    <div class="meter temp"><span style="width: {tempPct}%"></span></div>
    <span class="v">{Math.round(wheel.tempC)}</span>
  </div>
  <div class="row">
    <span class="label2">Brake °C</span>
    <div class="meter temp"><span style="width: {brakeTempPct}%"></span></div>
    <span class="v">{Math.round(wheel.brakeTempC)}</span>
  </div>
</div>
