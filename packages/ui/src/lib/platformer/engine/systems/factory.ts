...
import type { EntityKind, EntitySpawn, MapDefinition, TileKind } from '../../types.js';
...
export function isQuestionBlockReserveSpawn(map: MapDefinition, spawn: EntitySpawn): boolean {
  ...
  const tileAtSpawn = map.tiles[row]?.[col];
  return tileAtSpawn != null && shouldDeferQuestionLoot(spawn, tileAtSpawn);
}
